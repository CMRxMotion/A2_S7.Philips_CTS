...
class EfficientNet(BaseBackbone):
    ...
    def __init__(self,
        ...
        if 'classification' == head_type:
            if '2d' in conv_cfg['type']:
                ...
                aap = nn.AdaptiveAvgPool3d((1, 1, 1))
            else:
                aap = nn.AdaptiveAvgPool1d((1,))
            self.head_avgpool = aap
            self.head_fc = nn.Linear(self.out_channels, num_classes)
            self.head_type = 'classification'
        else:
            self.head_type = None
    ...
    def make_layer(self):
        ...
        if '3d' in self.conv_cfg['type']:
            se_conv_cfg = dict(type='Conv3d')
        elif '2d' in self.conv_cfg['type']:
            se_conv_cfg = dict(type='Conv2d')
        else:
            se_conv_cfg = dict(type='Conv1d')
    ...
    def forward(self, x):
        ...
        if self.head_type is not None:
            if 'classification' == self.head_type:
                outs = outs[-1]
                outs = self.head_avgpool(outs)
                outs = torch.flatten(outs, 1)
                outs = self.head_fc(outs)
                return outs
